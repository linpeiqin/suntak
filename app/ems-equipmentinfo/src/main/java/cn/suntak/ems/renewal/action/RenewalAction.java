package cn.suntak.ems.renewal.action;
 ...
 * @since 1.0.0
 */
public class RenewalAction extends EMSCRUD2Action<Renewal, RenewalService>{

	protected void save(HttpServletRequest request, ActionForm form, Renewal domain) throws Exception {
		if (domain.getNumber()==null || domain.getNumber().equals("")) {
			String number = this.fileNumberService.doCreateNumber("GZBH");
			domain.setNumber(number);
		}
		super.save(request, form, domain);
	}
	protected void afterSaveByAjax(HttpServletRequest request, ActionForm form, Renewal domain, cn.oz.json.JSONObject json) throws Exception {
    ...
    @Override
	protected Renewal getDomain(HttpServletRequest request, ActionForm form) throws Exception {
		Renewal domain = super.getDomain(request,form);
		...
		equipmentLifeCycle.setRenewal(domain);
	...
	@Override
	protected void afterCreate(HttpServletRequest request, ActionForm form, Renewal domain) throws Exception {
		...
		if (!equipmentId.equals(-1L)) {
			...
			request.setAttribute("equipmentNos",this.TranArray(eqs));request.setAttribute("contractNumber",domain.getContractNumber());
		}

	}

	@Override
	protected void renderForm(HttpServletRequest request, ActionForm form, Renewal domain, boolean canEdit) throws Exception {
		if (domain.getEquipmentLifeCycle()  != null) {
	...
	protected void afterSubmitEBS(Long id, String fileName, cn.oz.json.JSONObject json) {
		Renewal renewal = this.getService().load(id);
		String contactNo = renewal.getContractNumber();
		Set<EquipmentDetails> equipments = renewal.getEquipmentLifeCycle().getEquipmentDetails();
