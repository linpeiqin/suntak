package cn.suntak.ems.intofactoryandcheck.action;
 ...
 * @since 1.0.0
 */
public class IntoFactoryAndCheckAction extends EMSCRUD2Action<IntoFactoryAndCheck, IntoFactoryAndCheckService> {

    @Override
    protected void afterSubmitEBS(Long id, String fileName, cn.oz.json.JSONObject json) {
        IntoFactoryAndCheck intoFactoryAndCheck = this.getService().load(id);
        Set<EquipmentDetails> eqs = intoFactoryAndCheck.getEquipmentLifeCycle().getEquipmentDetails();
        for(EquipmentDetails eq : eqs) {
            eq.setType(2);
            eq.setEquipmentName(intoFactoryAndCheck.getFixedAssetsName());
            this.equipmentDetailsService.save(eq);
        }
        super.afterSubmitEBS(id,fileName,json);
    }
    @Override
    protected void renderForm(HttpServletRequest request, ActionForm form, IntoFactoryAndCheck domain, boolean canEdit) throws Exception {
    ...
    @Override
    protected IntoFactoryAndCheck getDomain(HttpServletRequest request, ActionForm form) throws Exception {
        IntoFactoryAndCheck domain = super.getDomain(request,form);
    ...
    equipmentLifeCycle.setIntoFactoryAndCheck(domain);
    domain.setEquipmentLifeCycle(equipmentLifeCycle);
    return domain;
}

    @Override
    protected void afterCreate(HttpServletRequest request, ActionForm form, IntoFactoryAndCheck domain) throws Exception {
        ...
        if(equipmentDetails.containLifeCycle("intoFactory","intoFactoryAndCheck")){
            throw new OzException("设备已经填写过验收单了");
        }
    ...
    protected void save(HttpServletRequest request, ActionForm form, IntoFactoryAndCheck domain) throws Exception {
        if (domain.getNumber()==null || domain.getNumber().equals("")) {
            String number = this.fileNumberService.doCreateNumber("RCYSBH");
            domain.setNumber(number);
        }
        super.save(request, form, domain);
    }
    protected void afterSaveByAjax(HttpServletRequest request, ActionForm form, IntoFactoryAndCheck domain, cn.oz.json.JSONObject json) throws Exception {
