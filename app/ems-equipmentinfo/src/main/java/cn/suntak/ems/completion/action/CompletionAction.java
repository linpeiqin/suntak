package cn.suntak.ems.completion.action;
 ...
 * @since 1.0.0
 */
public class CompletionAction extends EMSCRUD2Action<Completion, CompletionService>{
    ...
    protected void save(HttpServletRequest request, ActionForm form, Completion domain) throws Exception {
    ...
    protected void afterSaveByAjax(HttpServletRequest request, ActionForm form, Completion domain, cn.oz.json.JSONObject json) throws Exception {
        json.put("number",domain.getNumber());
    }

    @Override
    protected Completion getDomain(HttpServletRequest request, ActionForm form) throws Exception {
        Completion domain = super.getDomain(request, form);
        ...
        equipmentLifeCycle.setCompletion(domain);
        domain.setEquipmentLifeCycle(equipmentLifeCycle);
        return domain;
    }

    @Override
    protected void afterCreate(HttpServletRequest request, ActionForm form, Completion domain) throws Exception {
        Long equipmentId = RequestUtils.getLongParameter(request, "equipmentId", -1L);
        EquipmentDetails equipmentDetails = this.equipmentDetailsService.load(equipmentId);
        if(equipmentDetails.containLifeCycle("completion")){
            throw new OzException("设备已经填写过完工单了");
        }
