	...
	<table width="600" style="background-color:#FFFFFF" >
		<tr>
			...
			<td width="140" valign="top" class="oz-property">
				...
				<select id="user" name="user" style="width:100%;height:250px;" multiple="multiple" ondblclick="OZ.SELECT.appendSelected('user', 'users', false)">
				</select>
			</td>
			<td width="70px" style="text-align:center; vertical-align:middle;">
				<input id="btnAdd" type="button" class="oz-form-button" value=" <oz:messageSource code="oz.add"/> > " onclick="OZ.SELECT.appendSelected('user', 'users', false)"/><br /><br /><br />
				<input id="btnDelete" type="button" class="oz-form-button" value=" < <oz:messageSource code="oz.delete"/> " onclick="OZ.SELECT.removeSelected('users')"/><br /><br /><br />
				<input id="btnClear" type="button" class="oz-form-button" value=" << <oz:messageSource code="oz.clearall"/> " onclick="OZ.SELECT.removeAll('users')"/>
			</td>
			<td width="140px" valign="top">
				<oz:select value="users" property="users" options="userOptions" multiple="true" style="width:100%;height:263px;" ondblclick="OZ.SELECT.removeSelected('users')"></oz:select>
			</td>
		</tr>
	</table>
</body>
		...
		function(json){
			...
			if(allUsers.length > 0){
				for(var i = 0; i < allUsers.length; i++){
					selOptionObj.options[selOptionObj.length] = new Option(allUsers[i].name, allUsers[i].id);
				}
			}
		}
	);
...
function ozDlgOkFn(){
	var selOptionObj = document.getElementById("users");
	var option = null;
