...
$.validationEngine = {
	...
	validateCall : function(caller,rules) {	// EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FIELD
		...
		function _required(caller,rules){   // VALIDATE BLANK FIELD
			if("none" == $(caller).css("display"))
				return;
			
			callerType = $(caller).attr("type");
			var isTextArea = $(caller).is("textArea");
			if (callerType == "text" || callerType == "password" || callerType == "textarea" || isTextArea){
				if(!$(caller).val() || $.trim($(caller).val()).length == 0){					
	...
	buildPrompt : function(caller,promptText,type,ajaxed) {			// ERROR PROMPT CREATION AND DISPLAY WHEN AN ERROR OCCUR
		...
		if($.validationEngine.settings.containerOverflow){		// Is the form contained in an overflown container?
			...
			$($(caller).closest(".oz-panel-body, #page-center")[0]).append(divFormError);
		}
	...
	calculatePosition : function(caller,promptText,type,ajaxed,divFormError){
		...
		if($.validationEngine.settings.promptPosition == "topRight"){ 
			if($.validationEngine.settings.containerOverflow){		// Is the form contained in an overflown container?
				callerleftPosition += 0; // callerWidth -30;
			}else{
				callerleftPosition +=  0; //callerWidth -30; 
		...
		// FIXME 修正滚动条
		callerTopPosition += $("#page-center").scrollTop();
		// callerTopPosition -= $("#page-top").height();
		if($(caller).closest(".oz-panel-body").length > 0){
			callerTopPosition -= ($(".oz-tabs-header").height() + 6);
			callerleftPosition -= 10;
		}
		return {
