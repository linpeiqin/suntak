...
OZ.TB.prototype = {
	...
	init: function(config){
		...
		if(container){
			...
			if(container.className != this.cls){
				this.container.addClass(this.cls);// 更新容器的样式
			}
		}else{
			this.container = $(OZ.String.format(OZ.TB.Template.CONTAINER,this.id,this.cls,"oz-tb-text-align-"+this.buttonAlign));
	...
	render: function(){
		...
		for(var i = 0;i<this.buttons.length;i++){
			if(this.buttons[i].type == "seperator"){//按钮分隔条
				tplData.btns += OZ.String.format(OZ.TB.Template.BTN_SEPERATOR,this.buttons[i].id);
			}else if(this.buttons[i].type == "search"){//搜索按钮
				if(!(this.buttons[i].location)){
					this.buttons[i].location = "right";//默认为右侧放置
				}
				...
				tplData.btns += OZ.String.format(OZ.TB.Template.BTN,
					this.buttons[i].id,
					this.buttons[i].hidden ? "oz-btn oz-btn-hidden" : "oz-btn",
					this.buttons[i].iconCls ? OZ.String.format(OZ.TB.Template.BTN_ICON,this.buttons[i].iconCls) : "",
					this.buttons[i].text ? OZ.String.format(OZ.TB.Template.BTN_TEXT,this.buttons[i].text) : ""
				);
			}
		}
		...
		for(var j = 0;j<this.buttons.length;j++){
			if(this.buttons[j].type == "search" && this.buttons[j].location == "right"){//搜索按钮(进处理放在右侧的情况)
				searchBtns += this.getSearchBtnHtml(this.buttons[j]);
			}
		}
		if(searchBtns.length > 0){
			searchBtns = OZ.String.format(OZ.TB.Template.INNER,"oz-btns-align-right",searchBtns);
		}
		// 构建完整的DOM
		this.container.append(OZ.String.format(OZ.TB.Template.INNER,tplData.alignCls,tplData.btns) + searchBtns);
		// 绑定事件处理
		this.bindEvents();
		// 显示
		this.show();
	...
	getSearchBtnHtml: function(btnConfig){
		...
		if(jQuery.inArray("search",searchBtnTypes)!= -1){
			html += OZ.String.format(OZ.TB.Template.BTN_SEARCH_BTN,"simple","搜索");
		}
		if(jQuery.inArray("advance",searchBtnTypes)!= -1){
			html += OZ.String.format(OZ.TB.Template.BTN_SEARCH_BTN,"advance","高级搜索");
		}
		if(jQuery.inArray("clean",searchBtnTypes)!= -1){
			html += OZ.String.format(OZ.TB.Template.BTN_SEARCH_BTN,"clean","清空");
		}
		html = OZ.String.format(OZ.TB.Template.BTN_SEARCH,
			btnConfig.id,
	...
	hideRepeatSeperators: function(){
		...
		if(first == -1 || first == this.buttons.length - 1){
			return;
		}
