...
window.ozlog = (function($){
	...
	function init(){
		...
		// 使控制台可以移动
		try{
			$("#ozlogPanel").draggable({handle:"#ozlogHeader"});
		}catch(e){
	...
	function out(){
		...
		if(ozlog[arguments[0] + "Enable"] && visible){
			...
			var html = OZ.String.format(msgTpl,(++num),arguments[0],OZ.String.format.apply(null,args));
			if(desc){
