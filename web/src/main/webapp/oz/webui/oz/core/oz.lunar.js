...
function solarDay1(){
    ...
    var tt = '【'+Animals[(_lunarYear-4)%12]+'】' + cyclical(lDObj.monCyl) + '月 ' + cyclical(lDObj.dayCyl++) + '日' ;
    return (cl + tt + '</font>');
}
function solarDay2(){
    var sDObj = new Date(_lunarYear, _lunarMonth, _lunarDay);
    ...
    var tt = cyclical(_lunarYear-1900+36) + '年 ' + cDay(lDObj.month, lDObj.day);
	return(cl + tt + '</font>');
}
function solarDay3(){
	...
	for(i in sFtv){
		if(sFtv[i].match(/^(\d{2})(\d{2})([\s\*])(.+)$/)){
			tmp1 = Number(RegExp.$1)-(_lunarMonth+1);
			tmp2 = Number(RegExp.$2)-_lunarDay;
			if(tmp1 == 0 && tmp2 == 0) 
	...
	// 节气
	tmp1 = new Date((31556925974.7*(_lunarYear-1900) + sTermInfo[_lunarMonth*2+1]*60000) + Date.UTC(1900,0,6,2,5));
	tmp2 = tmp1.getUTCDate();
	if (tmp2 == _lunarDay) 
		solarTerms = solarTerm[_lunarMonth*2+1];  
	tmp1 = new Date((31556925974.7*(_lunarYear-1900) + sTermInfo[_lunarMonth*2]*60000) + Date.UTC(1900,0,6,2,5));
	tmp2= tmp1.getUTCDate();
	if (tmp2 == _lunarDay) 
		solarTerms = solarTerm[_lunarMonth*2]; 
